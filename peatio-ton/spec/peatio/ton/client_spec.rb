...
RSpec.describe Peatio::Ton::Client do
  let(:uri) { "http://admin:admin@127.0.0.1:33310" }
  let(:uri_without_authority) { "http://127.0.0.1:33310" }

  before(:all) { WebMock.disable_net_connect! }
  after(:all) { WebMock.allow_net_connect! }

  subject { Peatio::Ton::Client.new(uri) }
    ...
    context :methodnotfound do
      ...
      it do
        expect { subject.json_rpc(:methodnotfound) }.to \
          raise_error(Peatio::Ton::Client::ResponseError, "Method not found (-32601)")
      end
    end
    ...
    context :notfound do
      ...
      it do
        expect { subject.json_rpc(:notfound) }.to \
          raise_error(Peatio::Ton::Client::Error)
      end
    end
    ...
    context :connectionerror do
      ...
      it do
        expect { subject.json_rpc(:connectionerror) }.to \
          raise_error(Peatio::Ton::Client::ConnectionError)
      end
    end
  end
end
