...
RSpec.describe Peatio::Ton::Blockchain do
  context :features do
    it "defaults" do
      blockchain1 = Peatio::Ton::Blockchain.new
      expect(blockchain1.features).to eq Peatio::Ton::Blockchain::DEFAULT_FEATURES
    end

    it "override defaults" do
      blockchain2 = Peatio::Ton::Blockchain.new(cash_addr_format: true)
      expect(blockchain2.features[:cash_addr_format]).to be_truthy
    end

    it "custom feautures" do
      blockchain3 = Peatio::Ton::Blockchain.new(custom_feature: :custom)
      expect(blockchain3.features.keys).to contain_exactly(*Peatio::Ton::Blockchain::SUPPORTED_FEATURES)
    end
  end

  context :configure do
    let(:blockchain) { Peatio::Ton::Blockchain.new }
    ...
    it "currencies and server configuration" do
      currencies = [{id: :ton,
                      ...
                      options: {}}]
      settings = {server: "http://admin:admin@127.0.0.1:33310",
  ...
  context :latest_block_number do
    ...
    let(:server) { "http://admin:admin@127.0.0.1:33310" }
    let(:server_without_authority) { "http://127.0.0.1:33310" }
    ...
    let(:blockchain) do
      Peatio::Ton::Blockchain.new.tap {|b| b.configure(server: server) }
    end
    ...
    it "raises error if there is error in response body" do
      stub_request(:post, "http://127.0.0.1:33310")
        .with(body: {jsonrpc: "1.0",
                      method: :getblockcount,
                      params:  []}.to_json)
  ...
  context :build_transaction do
    ...
    context "three vout tx" do
      ...
      let(:blockchain) do
        Peatio::Ton::Blockchain.new.tap {|b| b.configure(currencies: [currency]) }
      end
    ...
    context "multiple currencies" do
      ...
      let(:blockchain) do
        Peatio::Ton::Blockchain.new.tap do |b|
  ...
  context :fetch_block! do
    ...
    let(:server_without_authority) { "http://127.0.0.1:33310" }
    let(:blockchain) do
      Peatio::Ton::Blockchain.new.tap {|b| b.configure(server: server, currencies: [currency]) }
    end
  ...
  context :load_balance_of_address! do
    ...
    context "address with balance is defined" do
      it "requests rpc listaddressgroupings and finds address balance" do
        ...
        result = blockchain.load_balance_of_address!(address, :ton)
    ...
    context "address is not defined" do
      it "requests rpc listaddressgroupings and do not find address" do
        ...
        expect { blockchain.load_balance_of_address!(address, :ton) }
    ...
    context "client error is raised" do
      ...
      it "raise wrapped client error" do
        expect { blockchain.load_balance_of_address!("anything", :ton) }
